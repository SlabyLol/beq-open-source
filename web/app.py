...
def load_model():
    ...
    try:
        ...
        tokenizer = CharTokenizer(ckpt.get("chars") or ckpt.get("vocab") or "")
        model = BeqTransformer(
            vocab_size=config["vocab_size"], d_model=config["d_model"],
            n_heads=config["n_heads"], n_layers=config["n_layers"],
            block_size=config["block_size"], dropout=config.get("dropout", 0.1),
        )
...
@app.post("/api-dashboard/keys/delete")
async def api_delete_key(request: Request, key_id: int = Form(...)):
    user = _user(request)
    if not user:
        return RedirectResponse("/login", status_code=303)
...
async def login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    resp.set_cookie("beq_session", session, httponly=True, samesite="lax", max_age=60 * 60 * 24 * 14)
    return resp
...
def _answer(prompt, max_tokens, temperature, preamble):
    """Knowledge → math → web search → model (reject identity echo on factual Qs)."""
    ...
    if _is_identity_fluff(completion) or (
        ...
        if _is_identity_fluff(completion):
            return prompt, (
                "I am Beq. For factual questions I normally search the web — "
                "search returned no result this time. Try rephrasing "
                '(e.g. "What is the Sun?") or ask the admin to turn Immer suchen ON.'
            )
...
async def chat(
    ...
    return templates.TemplateResponse(
        request, "index.html", _ctx(request, prompt=prompt, answer=answer, error=error, result=result)
    )
...
async def api_generate(request: Request, req: GenerateRequest, authorization: str | None = Header(default=None)):
    ...
    return {"prompt": req.prompt, "generated": full_prompt + new_text if not new_text.startswith(full_prompt) else new_text, "completion": new_text, "model": "beq"}
...
async def admin_page(request: Request):
    ...
    log_tail = ""
    if TRAIN_LOG.exists():
        log_tail = "\n".join(TRAIN_LOG.read_text(encoding="utf-8", errors="ignore").splitlines()[-40:])
