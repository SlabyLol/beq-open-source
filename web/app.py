...
def load_model():
    global model, tokenizer, CHECKPOINT, _load_error
    try:
        CHECKPOINT = _resolve_checkpoint()
        if not CHECKPOINT.exists():
            print(f"[load_model] no checkpoint at {CHECKPOINT}")
            model = tokenizer = None
            _load_error = f"missing file {CHECKPOINT}"
            return
        _load_error = None
        ...
        if tok_path.exists():
            ...
            print(f"[load_model] tokenizer {tok_path.name} vocab={tokenizer.vocab_size}")
        else:
        ...
        max_seq_len = int(config.get("max_seq_len") or config.get("block_size") or 128)
...
def _answer(prompt, max_tokens, temperature, preamble):
    ...
    if model is None or tokenizer is None:
        ...
        return prompt, (
            ...
            '(e.g. "What is the Sun?") or turn Crawler ON in admin.'
        )
    ...
    if _is_identity_fluff(completion) and not _is_identity_question(prompt):
        ...
        return prompt, (
            ...
            "Try rephrasing or add Q/A in knowledge.sbe."
        )
...
async def chat(
    ...
    if not mode_cfg.get("public_chat", True) and not _is_admin(user):
        error = "Chat is restricted. Set AI mode to Default in /admin."
    elif (
        model is None
...
async def api_generate(
    ...
    if (
        ...
        return JSONResponse({"error": "Model not loaded", "load_error": _load_error}, status_code=503)
    if user and not _is_admin(user):
